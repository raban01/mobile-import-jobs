...
@Service("baseStepService")
public abstract class BaseStepServiceImpl {

	private static final Logger LOGGER = LoggerFactory.getLogger(BaseStepServiceImpl.class);
	...
	protected abstract void hiveStep(int step) throws Exception;
	protected abstract void cassandraStep(int step) throws Exception;

	
	protected void startProcess(JobWithSteps job, String jobName, StringBuilder emailBody) throws Exception{
		LOGGER.info(jobName+ ": start.");
		JobHistory jobHistory=new JobHistory();
		jobHistory.setJobName(jobName);
		jobHistory.setTime(UUIDs.timeBased());
		JobMode jobMode=job.getJobMode();
		
		jobName=envName+"-"+jobName+"-"+jobMode.getCode();
		jobHistory.setJobFullName(jobName);
		jobHistory.setJobMode(jobMode.getCode());
		jobHistory.setStep(job.getStep());
